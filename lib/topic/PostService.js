...
export default class TopicService extends SupabaseService {
    ...
    async fetchGlobalTopics(lastTopicCreatedAt) {
        const data = await this.safeFetch((b) => b.select(this.selectQuery).gt("created_at", lastTopicCreatedAt ?? "1970-01-01T00:00:00.000Z").order("created_at", { ascending: false }));
        return data ?? [];
    }
}
