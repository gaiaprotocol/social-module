...
export default class ChatMessageForm extends UploadForm {
    ...
    constructor(tag, focus = true) {
        ...
        })), this.form = el("form", el(".input-container", this.input = new FileDropArea({ tag: "p.message-input", contenteditable: true }, (files) => this.appendFiles(files)), { click: () => this.input.domElement.focus() }), this.sendButton = new Button({
