import { AdaptiveModal, Button, ButtonType, DomNode, el, MaterialIcon, } from "@common-module/app";
...
class GifSearchBar extends DomNode {
    ...
    constructor() {
        ...
        this.append(el("label", new MaterialIcon("search"), this.input = el("input", { placeholder: "Search for GIFs" })));
...
export default class SelectGifModal extends AdaptiveModal {
    ...
    constructor() {
        ...
        this.main.append(el(".gif-search-bar-container", new Button({
            type: ButtonType.Circle,
            icon: new MaterialIcon("arrow_back"),
            click: () => this.searchBar.search(""),
        }), this.searchBar = new GifSearchBar()), this.categoryGrid = el(".category-grid"), this.gifGrid = el(".gif-grid"));
    ...
    renderCategories() {
        this.categoryGrid.empty().append(...categories.map((c) => el(".category", el("h4.title", c.name), { click: () => this.searchBar.search(c.name) }).style({
            backgroundImage: `url(${c.thumbnail})`,
        })));
    }
    resizeListener = () => this.renderGifs();
    renderGifs() {
        if (this.searchQuery) {
            const width = this.main.rect.width;
            this.deleteGrid = renderGrid({
                width,
                fetchGifs: (offset) => this.gf.search(this.searchQuery ?? "", { offset, limit: 25 }),
...
//# sourceMappingURL=SelectGifModal.js.map