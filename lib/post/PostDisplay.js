import { AvatarUtil, DateUtil, el, Icon, RichDisplay, } from "@common-module/app";
...
export default class PostDisplay extends SocialComponent {
    ...
    constructor(post, postService, options, interactions) {
        ...
        if (options.inView) {
            this.append(el("header", authorAvatar, authorDisplay, ownerMenuButton), messageDisplay, richDisplay, dateDisplay, actions);
        }
        else {
            this.append(authorAvatar, el("main", el("header", authorDisplay, ownerMenuButton), messageDisplay, richDisplay, dateDisplay, actions)).onDom("click", () => interactions.openPostView(post));
        }
