...
export default class PostList extends SocialComponent {
    ...
    async _fetchPosts() {
        ...
        const postIds = fetchedPosts.flatMap((item) => item.posts.map((post) => post.id));
        const repostedPostIds = this.options.signedUserId
            ? await this.postService.fetchUserRepostedPosts(postIds, this.options.signedUserId)
            : [];
        const likedPostIds = this.options.signedUserId
            ? await this.postService.fetchUserLikedPosts(postIds, this.options.signedUserId)
            : [];
        return {
            fetchedPosts,
            repostedPostIds,
    ...
    async refresh() {
        const cachedPosts = this.store.get("cached-posts") ?? [];
        const { fetchedPosts, repostedPostIds, likedPostIds, } = await this._fetchPosts();
